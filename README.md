...
- `site-content.js`: compatibility export surface for service content kept at the project root.
...
- Content-first routes can pull from `src/lib/data/content.js`; the underlying service, keyword-service, blog, skill, location, and FAQ records live in focused modules under `src/lib/data/`.
...
- Global CSS favors reusable classes over component-scoped CSS and is split into ordered foundation, navigation, hero, component, route, animation, and responsive sections under `src/lib/styles/`.

## How To Extend

- Add a new core service in `src/lib/data/services.js` or a generated keyword service in `src/lib/data/keyword-services.js`, then confirm it appears in `/services/`, the service nav, sitemap, footer, and relevant internal links.
- Add a new skill in `src/lib/data/skills.js`, then connect it to service pages through `serviceSkillMap`.
- Add a new blog post in `src/lib/data/blog.js`, then link it from relevant services and troubleshooting cards.
- Add a new location in `src/lib/data/locations.js`, including nearby locations and related services.
- For new visual card icons, extend `ServiceIcon.svelte` and add only the minimum CSS needed in the relevant `src/lib/styles/` section.
